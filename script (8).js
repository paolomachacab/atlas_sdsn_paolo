...
async function loadAvailableIndicators() {
  ...
  // Group by ODS (primer segmento del nombre de archivo antes del primer _)
  const groups = {};
  ...
  const groups = {};
  for (const ind of INDICATOR_FILES) {
    const ods = ind.file.substring(0, 2);
    if (!groups[ods]) groups[ods] = [];
    groups[ods].push(ind);
  }

  for (const [ods, inds] of Object.entries(groups)) {
    const grp = document.createElement('optgroup');
    grp.label = ODS_NAMES[ods] || `ODS ${parseInt(ods)}`;
...
async function readExcel(filename) {
  ...
  const result = { mun:new Map(), dep:new Map(), thresholds:null, colLabels:{ v1:'2012', v2:'2024' } };
...
function renderLegend(data) {
  ...
  for (const [,entry] of mun) counts[classify(entry.v2 ?? entry.v1, data.thresholds)]++;
