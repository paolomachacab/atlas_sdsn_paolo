...
function classify(value, thresholds) {
  ...
  // Determinar dirección: si meta_max < meta_min → menor es mejor (descendente)
  // Ejemplo hacinamiento: meta_max=0, meta_min=15 → descendente
  const ascending = t.meta.max >= t.meta.min;

  if (ascending) {
    // Mayor es mejor (ej. alfabetismo, cobertura agua)
    // Usar los MIN de cada categoría como umbral inferior
    if (value >= t.meta.min)    return 'meta';
    if (value >= t.proximo.min) return 'proximo';
    if (value >= t.retos.min)   return 'retos';
    return 'grandes';
  } else {
    // Menor es mejor (ej. hacinamiento, abandono escolar, pobreza)
    // Usar los MIN de cada categoría como umbral superior (son los valores más altos del rango)
    if (value <= t.meta.min)    return 'meta';
    if (value <= t.proximo.min) return 'proximo';
    if (value <= t.retos.min)   return 'retos';
    return 'grandes';
