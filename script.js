...
const INDICATOR_FILES = [
  { file: '04_01_02_01_ash.xlsx', label: 'ASH · 04.01.02.01' },
  { file: '04_01_02_02_asm.xlsx', label: 'ASM · 04.01.02.02' },
  { file: '04_02_02_03_pfp.xlsx', label: 'PFP · 04.02.02.03' },
  { file: '04_03_04_04_pes.xlsx', label: 'PES · 04.03.04.04' },
  { file: '04_06_01_06_alf.xlsx', label: 'ALF · 04.06.01.06' },
  { file: '04_10_01_09_pci.xlsx', label: 'PCI · 04.10.01.09' },
  { file: '04_10_01_09_pcs.xlsx', label: 'PCS · 04.10.01.09' },
  { file: '05_01_01_01_pga.xlsx', label: 'PGA · 05.01.01.01' },
  { file: '05_05_01_05_acm.xlsx', label: 'ACM · 05.05.01.05' },
  { file: '11_01_01_01_thm.xlsx', label: 'THM · 11.01.01.01' },
  { file: '16_07_02_03_tcs.xlsx', label: 'TCS · 16.07.02.03' },
];
...
function initMaps() {
  ...
  mapLeft = L.map('map-left', { zoomControl: true,  attributionControl: false })
              .setView(BOLIVIA_CENTER, BOLIVIA_ZOOM);
  mapRight = L.map('map-right', { zoomControl: false, attributionControl: false })
               .setView(BOLIVIA_CENTER, BOLIVIA_ZOOM);
...
async function loadGeoJSON() {
  ...
  // Cargar municipios (tu shapefile convertido)
  try {
    const munRes = await fetch(GEOJSON_MUN);
    if (munRes.ok) {
      state.geoMun = await munRes.json();
      // Detectar campos automáticamente desde el shapefile convertido
      GEO_FIELDS = detectFields(state.geoMun);
    }
  } catch (e) {
    console.warn('GeoJSON municipios no disponible:', e);
    state.geoMun = null;
  }

  // Cargar departamentos (opcional)
  try {
    const depRes = await fetch(GEOJSON_DEP);
    if (depRes.ok) state.geoDep = await depRes.json();
  } catch (e) {
    console.warn('GeoJSON departamentos no disponible (opcional):', e);
    state.geoDep = null;
  }
...
function getFeatureId(feature) {
  ...
  // Usar campo detectado automáticamente, con fallbacks
  return String(
    p[GEO_FIELDS.idField] ??
...
function renderMaps() {
  ...
  try {
    ...
    if (bounds.isValid()) {
      ...
      mapRight.fitBounds(bounds, { padding: [20, 20] });
    }
  } catch (e) {}
}
...
async function init() {
  ...
  dom.btnDep.addEventListener('click',       () => onLevelChange('departamental'));
